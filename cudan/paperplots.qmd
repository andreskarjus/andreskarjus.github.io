...
tops = attr(festgenrevecs, "tops") %>% 
  ...
  ungroup() %>% 
  mutate(colorgenre=case_when(txtKind%in% levels(festgenrevecs$colorgenre)~txtKind, T~"...")) %>% 
  mutate(colorgenre2=fct_relevel(as.factor(colorgenre), 
                                levels(festgenrevecs$colorgenre) %>% gsub("other","...",.)))
  

ggplot(tops, aes(x=colorgenre2, y=s, fill=colorgenre2))+
  geom_bar(stat="identity", width=1, color=NA)+
  scale_fill_manual(values=sixgcols)+
