...
abcols= divergingx_hcl(6, palette="Zissou 1")[c(1,5)] %>% c("gray2")
abshapes=c(16,4)
...
divyearplot = function(vecs, ex, shape=16, yl="", splitab=T, convert=T, networkstats=T, ylims=NULL){
  ...
  if(!splitab & !is.null(ex)){
   if(networkstats){
    ...
    g=
      g+facet_wrap(~libelleFestival_NEW_VZ, nrow = ceiling(length(ex)/6))+
...
sysplots = function(sysdiv, yl){
  ...
  g=  ggplot(sysdiv,aes(YearEvent, sysdiv, color=ab))+
    ...
    scale_color_manual(values=c(abcols) %>% {.[1:2]=lighten(.[1:2],0.2);. })+
    theme_bw()+
    theme(legend.position = 'none')+
    theme(
      panel.grid.minor.x  = element_blank(),
