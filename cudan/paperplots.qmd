...
# gender examples TODO------- aside from Cannes and Sundance (and instead of Hot Docs and Sitges), could we please have SXSW for B and Berlinale for A
...
divyearplot = function(vecs, ex, shape=16, yl="", splitab=T, convert=T, networkstats=T, ylims=NULL, genrecols=sixgcols, pies=NULL, nrowmod=6){
  if(splitab){
...
# content space with change plots
...
festrolevecs %>% filter(grepl("SXSW", libelleFestival_NEW_VZ) ) %>% as.data.frame() %>% arrange(YearEvent)

festrolevecs %>% filter(grepl("Sundance", libelleFestival_NEW_VZ) ) %>% as.data.frame() %>% arrange(YearEvent)

attr(festrolevecs, "sysdivs")

library(lme4)

dat = fesfilm2 %>% 
    rename(nsort=nroles) %>%  # variable passing now works
    mutate(tmp=paste0(TitleVA, "_", YearOfProduction )) %>% 
...
plotbees = function(vecs, tag, tlab="", col=NULL, cex=3, vw=1.7){
  g=ggplot(vecs, aes(x=as.factor(ab) %>% as.numeric , group=as.factor(ab) %>% as.numeric, color=ab,y=target))+
  #geom_boxplot(fill=NA, color="gray30", width=0.2)+
  geom_violin(fill=NA, color="gray50", size=0.1,scale="count", width=vw)
  
  if(is.null(col)){
    g=g+geom_beeswarm(aes(color=ab), cex = cex,priority = "random",size=0.5)
  } else {
...
fig_gex = divyearplot(festrolevecs  %>% mutate(target=ratiof, lower=rfminboot, upper=rfmaxboot),
            ex=exrolefests, yl="% of women", convert=convertplots, splitab=F, networkstats = F, ylims=c(0,0.59), nrowmod = 2)+theme(axis.title = element_blank())
...
fig_thematic_div = 
  ...
  tags(divyearplot(festgenrevecs  %>% mutate(target=indiv, lower=ilower, upper=iupper), 
            ex=exgenfests, yl="Internal thematic diversity", convert=F, splitab=F, networkstats = F, nrowmod = 2, pies = pies2, genrecols = sixgcols), "B")
) /
...
tbees = 
  plotbees(festcountryvecs %>% mutate(target=indiv), "C", tlab=" Internal", col="gray50", cex=3, vw = 1.5) +
  plotbees(festcountryvecs %>% mutate(target=exdiv), "D", tlab=" Contributing", col="gray50", cex=2.9, vw=1.4)+plot_layout(nrow=2)
...
fig_geo_div = 
  ...
  tags(divyearplot(festcountryvecs  %>% mutate(target=indiv, lower=ilower, upper=iupper), 
            ex=exgeofests, yl="Internal geographic diversity", convert=F, splitab=F, networkstats = F, nrowmod = 2, pies=piesg, genrecols = sixcols), "B")
) /
((tbees | tdiv2 | pspacer)+plot_layout(widths = c(0.3,0.7,0), guides = "keep"))+
...
fig_lang_div = 
  (tags(sysplots(attr(festlangvecs,"sysdivs"), yl="Circuit language diversity")+
          theme(legend.position = c(0.75,0.1),legend.direction = "horizontal", legend.title = element_blank()),"A") +
  tags(divyearplot(festlangvecs  %>% mutate(target=indiv, lower=ilower, upper=iupper), 
            ex=exlangfests, yl="Internal language diversity", convert=F, splitab=F, networkstats = F, nrowmod = 2, pies=piesl, genrecols = langcols), "B")
) /
((tbees | tdiv2 | pspacer)+plot_layout(widths = c(0.3,0.7,0), guides = "keep"))
