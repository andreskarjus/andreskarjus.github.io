...
abcols=divergingx_hcl(6, palette="Zissou 1")[c(1,5)]
abshapes=c(16,4)

sixgcols = c(divergingx_hcl(6, palette="Zissou 1") %>% {.[6]=darken(.[6],0.3);.},  "gray50") 
sixcols = c(divergingx_hcl(6, palette="Zissou 1") %>% {.[6]=darken(.[6],0.3);.}, "gray10",  "gray50") 
langcols= c(divergingx_hcl(6, palette="Zissou 1")%>% {.[6]=darken(.[6],0.3);.}, "gray10",  "gray50")[c(1,3,5,6,7,8)] 
...
examplefests = c("Festival de Cannes", "Sundance", "Tokyo IFF", "Zurich FF")
                 #"Hot Docs Festival", "Sitges FF", "Busan IFF", "BAFICI")
...
divyearplot = function(vecs, ex, shape=16, yl="", splitab=T, convert=T, networkstats=T){
  if(splitab){
  ...
  if(convert){
    ...
    ggplotly(g,tooltip="text") %>% config(doubleClickDelay=500)
  } else {g}
...
A listers are circles, B listers are crosses. 618 festivals, incl 132 series with 2+ events, 32700 unique films, 42365 festival-film pairs
...
divyearplot(nodelist  %>% mutate(target=degree, lower=NA, upper=NA), 
            ex=examplefests, yl="Raw degree", convert=convertplots, splitab=F, 
            networkstats = T)
...
{ggplot(hm , 
       aes(fromlibelle, tolibelle, fill=fillweight, text=tool))+
  ...
  #scale_fill_viridis_c(end=0.9, na.value = "white")+
    scale_fill_gradientn(colors = cls, na.value = "white")+
  theme_bw()+
    theme(legend.position = 'none', 
      panel.border = element_blank(),
      panel.grid.minor.x = element_blank(),
...
divyearplot(nodelist  %>% mutate(target=lndegree, lower=NA, upper=NA), 
            ex=examplefests, yl="Degree normalized", convert=F, splitab=F, 
            networkstats = T)
...
25674 unique films, 572 unique fests, 34858 filmfest pairs, after final filter.
...
# 110
fesfilm2 %>% 
  filter(LibelleEvent_NEW_VZ %in% "Tallinn_Black_Nights_IFF_2012") %>% 
  left_join(filmgen, by="refFilm") %>% 
  count(txtKind) %>% 
  ggplot(aes(y=txtKind, x=n))+
  geom_bar(stat="identity")+
  theme_bw()+
  theme(axis.title = element_blank())+
  coord_cartesian(expand=0)+
  labs(subtitle="Tallinn_Black_Nights_IFF_2012")
...
divyearplot(festgenrevecs  %>% mutate(target=indiv, lower=ilower, upper=iupper), 
            ex=examplefests, yl="Internal genre diversity", convert=T, splitab=F, networkstats = F)
...
divyearplot(festgenrevecs  %>% mutate(target=exdiv, lower=elower, upper=eupper), 
            ex=examplefests, yl="External genre diversity", convert=convertplots, splitab=F, networkstats = F)
...
divyearplot(yeardiffvals  %>% mutate(target=yearmean, lower=yearmean-yearsd, upper=yearmean+yearsd), 
            ex=examplefests, yl="Time distance", convert=F, splitab=F, networkstats = F)
...
24580 unique films, 543 unique fests, 42902 filmfest pairs, after final filter
...
divyearplot(festlangvecs  %>% mutate(target=indiv, lower=ilower, upper=iupper), 
            ex=examplefests, yl="Language diversity", convert=convertplots, splitab=F, networkstats = F)
...
divyearplot(festlangvecs  %>% mutate(target=exdiv, lower=elower, upper=eupper), 
            ex=examplefests, yl="External language diversity", convert=convertplots, splitab=F, networkstats = F)
...
26906 unique films, 580 unique fests, 52811 filmfest pairs, after final filter
...
divyearplot(festcountryvecs  %>% mutate(target=indiv, lower=ilower, upper=iupper), 
            ex=examplefests, yl="Internal geo diversity", convert=convertplots, splitab=F, networkstats = F)
...
divyearplot(festcountryvecs  %>% mutate(target=exdiv, lower=elower, upper=eupper), 
            ex=examplefests, yl="External geo diversity", convert=convertplots, splitab=F, networkstats = F)
...
divyearplot(festcountryvecs  %>% mutate(target=eventdist, lower=edlower, upper=edupper), 
            ex=examplefests, yl="distance", convert=convertplots, splitab=F, networkstats = F)
