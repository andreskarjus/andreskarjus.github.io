...
struct WallCollapseDepositBuffer {
  // Pending bare-dirt depth in millionths of a metre. A separate cell field
...
const WALL_COLLAPSE_DIRT_RECOVERY: f32 = 0.72;
const WALL_DIRT_DEPOSIT_QUANTIZATION: f32 = 1000000.0;

fn wallDamageQuantized(value: f32) -> u32 {
  return u32(clamp(round(max(0.0, value) * 1000000.0), 0.0, 4294967040.0));
}

fn queueWallDirtDepth(position: vec2i, depth: f32, n: i32) {
  let cell = vec2u(clamp(position, vec2i(0), vec2i(n - 1)));
  let amount = u32(clamp(
    round(max(0.0, depth) * WALL_DIRT_DEPOSIT_QUANTIZATION),
    0.0,
    4294967040.0
  ));
...
fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  if (integrity <= WALL_COLLAPSE_INTEGRITY) {
    ...
    let recoveredDirtDepth = proxyMasonryVolume
      * WALL_COLLAPSE_DIRT_RECOVERY
      / cellArea;
    queueWallDirtDepth(center, recoveredDirtDepth * 0.4, n);
    for (var direction = 0u; direction < 4u; direction = direction + 1u) {
      queueWallDirtDepth(
        center + cardinal[direction],
        recoveredDirtDepth * 0.15,
        n
      );
    }
    // The structural record becomes inactive immediately. Dirt owns the
    // resulting pile; no wall mesh, foundation, or hydraulic crest remains.
    block.gridFoundation.w = 0.0;
...
fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  // Every masonry block occupies one physical cell. Publishing all four sides
  // is conservative for arbitrary authored yaw and closes diagonal corner hops.
...
const WALL_MAX_DIRT_DEPOSIT_DEPTH_PER_PASS_Q: u32 = 1200000u;
...
fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  let granted = min(pending, WALL_MAX_DIRT_DEPOSIT_DEPTH_PER_PASS_Q);
  atomicSub(&collapseDeposits.values[index], granted);
  let depositDepth = f32(granted) / WALL_DIRT_DEPOSIT_QUANTIZATION;
  ...
  let nextDirtDepth = clamp(
    soilValue.dirtDepth + depositDepth,
    0.0,
    nextMovableDepth
  );
  let looseShare = clamp(
    depositDepth / max(nextMovableDepth, 0.0001),
    0.0,
    1.0
  );
  value.y = nextMovableDepth;
  soilValue.dirtDepth = nextDirtDepth;
  soilValue.compaction = mix(
...
struct WallRenderUniform {
  // 0 ordinary curtain/block, 1 tower; 2 is retained as an empty legacy mode.
  mode: u32,
  pad0: u32,
  pad1: u32,
  pad2: u32,
...
fn vertexMain(
  ...
  let isActive = block.gridFoundation.w >= 1.0;
