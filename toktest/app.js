const APP_DATA = window.__TIKTOK_DASHBOARD_DATA__;

if (!APP_DATA) {
  throw new Error("Dashboard data bundle was not loaded.");
}
...
function buildHeroStats() {
  ...
  const stats = [
    ...
    { label: "Map Layout", value: "Cached t-SNE" },
  ];
...
function renderMapPlot() {
  ...
  titleTarget.textContent = `${variableLabel(state.mapColorKey)} across the fixed corpus map`;
  ...
  Plotly.react("map-plot", data, mapLayout(), plotConfig);
  noteTarget.textContent = `${variableLabel(state.mapColorKey)} is drawn as discrete groups over the paper's shared coordinate space; ${state.mapSizeKey === "none" ? "point size is constant." : `point size is ${state.sizeClipEnabled ? "percentile-clipped" : "using the full observed range"}.`}`;
}
...
function renderDistributionPlot(targetId, key, slotLabel) {
  ...
  if (isNumeric(key)) {
    ...
    const layout = baseLayout({
      title: { text: `${variableLabel(key)} distribution`, font: { family: FONT_DISPLAY, size: 18 } },
  ...
  const layout = baseLayout({
    title: { text: `${variableLabel(key)} frequencies`, font: { family: FONT_DISPLAY, size: 18 } },
...
function renderNumericRelationship(aKey, bKey) {
  ...
  const layout = baseLayout({
    title: { text: `${variableLabel(aKey)} vs. ${variableLabel(bKey)}`, font: { family: FONT_DISPLAY, size: 16 } },
...
function renderMixedRelationship(aKey, bKey) {
  ...
  const layout = baseLayout({
    title: { text: `${variableLabel(numericKey)} by ${variableLabel(categoricalKey)}`, font: { family: FONT_DISPLAY, size: 16 } },
